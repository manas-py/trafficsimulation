...
The baseline simulation parameters were set as follows:
\begin{itemize}
    \item Number of lanes per direction: 2 (North, South, East, West)
    \item Green time per direction $g = 30$ s
    \item Bike phase (all red) $b = 15$ s
    \item Reaction delay $T = 1.2$ s
    \item Sensitivity $A = 0.8$ s$^{-1}$
    \item Arrival rate $\lambda = 20$ veh/min
    \item Capacity: 50 vehicles per lane
\end{itemize}

With these parameters, the saturation flow rate was computed as $s \approx 2050$ veh/h/lane (0.57 veh/s/lane), consistent with field observations. The cycle length is $C = 2g + b = 75$ s, with each direction experiencing a red time of $r = g + b = 45$ s.
...
Figure~\ref{fig:queue-time} displays the queue length evolution over time for the baseline parameters. The simulation exhibits characteristic oscillatory behavior, with queues building during red phases and clearing during green phases. The amplitude and frequency of these oscillations depend on the arrival rate, signal timing, and driver reaction parameters.
...
\begin{itemize}
    ...
    \item The bike phase (all red) causes additional queue buildup across all directions
...
\begin{enumerate}
    ...
    \item \textbf{Bike phase impact}: The 15-second bike phase (all red) affects all directions simultaneously, causing uniform queue buildup. This phase adds $b = 15$ s to the red time for each direction, increasing delay by approximately 20\% compared to a signal without a bike phase.
...
\begin{enumerate}
    ...
    \item \textbf{Bike phase effects}: The all-red bike phase creates periodic disruptions that affect all directions simultaneously, demonstrating how non-vehicle phases impact intersection capacity and delay.
\end{enumerate}
...
The addition of a bike phase demonstrates how non-vehicle signal phases impact intersection performance, showing that even a 15-second all-red period can increase average delay by approximately 20\%. This finding has practical implications for intersection design and signal timing optimization in bicycle-friendly urban environments.
